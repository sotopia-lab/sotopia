...
class MockClientSession:
    ...
    async def ws_connect(self, url: str, timeout: Optional[float] = None) -> MockWebSocket:
...
async def test_group_messaging(mock_websocket: MockWebSocket) -> None:
    ...
    for resp in additional_responses:
        assert resp["type"] == "SERVER_MSG", "Wrong response type in additional response"
    ...
    all_npc_ids: List[str] = [first_response["data"]["npc_id"]] + [r["data"]["npc_id"] for r in additional_responses]
...
async def test_targeted_messaging(mock_websocket: MockWebSocket) -> None:
    ...
    assert first_response["data"]["type"] == "npc_response", "Not an NPC response"
    first_npc_id = first_response["data"]["npc_id"]
    assert first_npc_id in ["agent1", "agent3"], f"NPC {first_npc_id} not in target list"
...
async def test_group_membership_overlap(mock_websocket: MockWebSocket) -> None:
    ...
    for group_name, expected_members in groups.items():
        ...
        assert first_response["data"]["type"] == "npc_response", "Not an NPC response"
        first_npc_id = first_response["data"]["npc_id"]
        assert first_npc_id in expected_members, f"NPC {first_npc_id} not in {group_name}"
...
async def test_multi_turn_conversation(mock_websocket: MockWebSocket) -> None:
    ...
    logger.info(f"Turn 1 - {first_response1['data']['npc_id']}: {first_response1['data']['content']}")
    ...
    all_npc_ids1: List[str] = [first_response1["data"]["npc_id"]] + [r["data"]["npc_id"] for r in additional_responses1]
    ...
    logger.info(f"Turn 2 - {first_response2['data']['npc_id']}: {first_response2['data']['content']}")
    ...
    logger.info(f"Turn 3 - {first_response3['data']['npc_id']}: {first_response3['data']['content']}")
    
    # Verify only the specific NPC responded
    assert first_response3["data"]["npc_id"] == specific_npc, f"Wrong NPC responded: {first_response3['data']['npc_id']}"
    ...
    logger.info(f"Turn 4 - {first_response4['data']['npc_id']}: {first_response4['data']['content']}")
    ...
    all_npc_ids4: List[str] = [first_response4["data"]["npc_id"]] + [r["data"]["npc_id"] for r in additional_responses4]
    ...
    logger.info(f"Turn 5 - {first_response5['data']['npc_id']}: {first_response5['data']['content']}")
    ...
    first_response6, additional_responses6 = await send_client_message(
        mock_websocket, 
        final_msg, 
        target_npcs=["agent1", "agent2", "agent3"]
    )
    
    # Log the conversation
    logger.info(f"Turn 6 - Client: {final_msg}")
    logger.info(f"Turn 6 - {first_response6['data']['npc_id']}: {first_response6['data']['content']}")
    for resp in additional_responses6:
        logger.info(f"Turn 6 - {resp['data']['npc_id']}: {resp['data']['content']}")
    
    # Verify all NPCs responded
    all_npc_ids6: List[str] = [first_response6["data"]["npc_id"]] + [r["data"]["npc_id"] for r in additional_responses6]
