...
async def publish_and_capture(
    redis_client, channel, message, response_channel, timeout=1.0
):
...
async def test_simulator_message_flow(redis_client):
    ...
    with patch(
        "redis_om.model.model.get_redis_connection", return_value=redis_client
...
async def test_redis_agent_group_message_processing(active_redis_agent, redis_client):
    ...
    command_channel = "command:test_conn_id"
    ...
    response = await publish_and_capture(
        redis_client, command_channel, command_data, output_channel, timeout=1.0
    )
...
async def test_redis_agent_epilog_publishing(active_redis_agent, redis_client):
    ...
    epilog_channel = "epilog:test_conn_id"
    ...
    try:
        ...
        with patch(
            ...
            try:
                ...
                moderator.handle_unified_message.return_value = type(
                    "MockObservations", (), unified_message_response
                )
                ...
                while asyncio.get_event_loop().time() - start_time < 1.0:
                    ...
                    if message and message["type"] == "message":
                        ...
                        if (
                            "action_type" in data.get("data", {})
                            and data["data"]["action_type"] == "unified_message"
                        ):
...
async def test_epilog_deduplication(redis_client):
    ...
    epilog1 = MagicMock()
    epilog1.model_dump_json = MagicMock(
        return_value=json.dumps({"messages": [["test", "data"]]})
    )

    epilog2 = MagicMock()
    epilog2.model_dump_json = MagicMock(
        return_value=json.dumps({"messages": [["test", "data"]]})
    )
    ...
    epilog3 = MagicMock()
    epilog3.model_dump_json = MagicMock(
        return_value=json.dumps({"messages": [["different"]]})
    )
