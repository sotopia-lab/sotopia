...
def mock_observations():
    ...
    with patch('sotopia.experimental.agents.moderator.Observations', autospec=True) as mock_obs_cls:
...
async def test_send_epilog_deduplication(test_moderator):
    ...
    assert test_moderator.send.call_count == 1  # Use call_count instead of assert_called_once
...
@pytest.mark.asyncio
async def test_handle_unified_message_direct_targeting(test_moderator, mock_observations):
...
@pytest.mark.asyncio
async def test_handle_unified_message_group_targeting(test_moderator, mock_observations):
    """Test the handle_unified_message method for group targeting"""
    # Set up groups
    test_moderator.groups = {
        "team_a": ["agent1"],
        "team_b": ["agent2"]
    }
...
async def test_astep_response_with_tracking(test_moderator):
    ...
    test_moderator.message_senders = {
