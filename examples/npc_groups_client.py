    ...
    try:
        while True:
            ...
            if choice == "1":
                ...
                group_id = input(f"Enter group ID ({', '.join(groups.keys())}): ").strip()
                if group_id not in groups:
                    print(f"Invalid group ID. Must be one of: {', '.join(groups.keys())}")
                    continue
...
async def automated_demo(websocket):
    ...
    try:
        ...
        await send_message_to_npcs(
            websocket, 
            "Hello everyone! Can you all introduce yourselves?"
        )
