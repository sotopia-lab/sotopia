...
class RedisAgent(BaseAgent[Observation, AgentAction]):
            ...
            try:
                ...
                async for msg in self.websocket:
                    ...
                    if msg.type == aiohttp.WSMsgType.TEXT:
                        ...
                        logger.debug(
                            f"Received message from WebSocket: {message_text[:100]}..."
                        )
                        ...
                        try:
                            ...
                            logger.error(
                                f"Failed to parse WebSocket message: {message_text[:100]}..."
                            )
                            await self.send_to_websocket(
                                {
                                    "type": "ERROR",
                ...
                if error_count >= max_errors:
                    logger.critical(
                        f"Too many errors ({error_count}), stopping WebSocket listener"
                    )
                    break
    ...
    async def process_start_message(self, message: Dict[str, Any]) -> None:
        ...
        logger.info(
            f"Sent start message to moderator with {len(self.active_npcs)} NPCs and {len(self.npc_groups)} groups"
        )
    ...
    async def process_client_message(self, message: Dict[str, Any]) -> None:
        ...
        logger.info("Routed client message to moderator for distribution")
    ...
    async def aact(self, obs: Observation) -> AgentAction | None:
        ...
        if obs.agent_name == "epilog":
            try:
                ...
                server_message = {
                    "type": "SERVER_MSG",
                    "data": {"type": "messages", "messages": epilog_data},
                }
                # Send to WebSocket
        ...
        # For responses from NPCs, forward to WebSocket
            ...
            try:
                ...
                logger.info(
                    f"Sent NPC response from {obs.agent_name} to WebSocket client"
                )
            except Exception as e:
