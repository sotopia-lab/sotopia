...
class BaseEpisodeLog(BaseModel):
    ...
    messages: list[list[tuple[str, str, str]]]  # Messages arranged by turn: (sender, receiver, content)
    reasoning: str = Field(default="")
    rewards: list[tuple[float, dict[str, float]] | float]  # Rewards arranged by turn
    rewards_prompt: str
    
    # Tracking for groups and message contexts
    groups: Dict[str, List[str]] = Field(default_factory=dict)  # Group name -> list of agent names
                ...
                try:
                    ...
                    if isinstance(message_data, dict) and "content" in message_data:
                        ...
                        if context == "group":
                            ...
                            messages_in_this_turn.append(f"{sender} to group {group_name}: {content}")
                        elif context == "individual":
                            # Format individual messages
                            target = receiver.replace("Agent:", "")
                            messages_in_this_turn.append(f"{sender} to {target}: {content}")
                        elif context == "response":
                            # Format responses
                            response_to = receiver.replace("Response:", "")
                            messages_in_this_turn.append(f"{sender} responds to {response_to}: {content}")
                        elif context == "broadcast":
                            # Format broadcast messages
                            messages_in_this_turn.append(f"{sender} (broadcast): {content}")
                        else:
                            # Default format for unknown context
                            messages_in_this_turn.append(f"{sender} to {receiver}: {content}")
                    else:
                        # Default handling
                        messages_in_this_turn.append(f"{sender} to {receiver}: {message}")
