from sotopia.database import EpisodeLog, AnnotationForEpisode, EnvironmentProfile
...
def aggregate_rewards(rewards: List[Tuple[float, Dict[str, float]]])-> Tuple[float, Dict[str, float]]:
    ...
    ret_rewards = (average_list([r[0] for r in rewards]), average_dict([r[1] for r in rewards]))
    return ret_rewards

def human_annotation_to_episodelog(human_annotation: list[AnnotationForEpisode], return_model_episodes: bool=False, aggregate: bool=False) -> dict[str, EpisodeLog | tuple[EpisodeLog, EpisodeLog]]:
    """
